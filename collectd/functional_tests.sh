...
retcode=$?
echo "PUTVAL $(hostname)/heat/commands-stack_get_token interval=900 N:$retcode" 
[ $retcode -ne 0 ] && echo "Unable to get token from keystone." 1>&2 && echo "PUTVAL $(hostname)/heat/commands-stack interval=900 N:$retcode" && exit 1
...
then
        ...
        "$HEAT_ENDPOINT/$OS_PROJECT_ID/stacks/$stackname"
        retcode=$?
        echo "PUTVAL $(hostname)/heat/commands-stack_deletion interval=900 N:$retcode" 
        [ $retcode -ne 0 ] && echo "Unable to delete stack $stackname." 1>&2 && echo "PUTVAL $(hostname)/heat/commands-stack interval=900 N:$retcode" && exit 1
        #SECONDS is bash special variable, it counts number of seconds from start of the script,
...
curl -X GET -g -k -q -s -L --fail -o /dev/null \
         ...
         "$HEAT_ENDPOINT/$OS_PROJECT_ID/stacks/$stackname"
retcode=$?
[ $retcode -ne 0 ] && echo "PUTVAL $(hostname)/heat/commands-stack_deletion interval=900 N:0" || echo "PUTVAL $(hostname)/heat/commands-stack_deletion interval=900 N:1"
[ $retcode -eq 0 ] && echo "Stack $stackname hasn't been deleted, still exists." 1>&2 && echo "PUTVAL $(hostname)/heat/commands-stack interval=900 N:1" && exit 1
...
retcode=$?
[ $retcode -eq 0 ] && echo "PUTVAL $(hostname)/heat/commands-stack_creation interval=900 N:0" || echo "PUTVAL $(hostname)/heat/commands-stack_creation interval=900 N:1"
[ $retcode -ne 0 ] && echo "Unable to create stack. Please check HEAT_JSON variable." 1>&2 && echo "PUTVAL $(hostname)/heat/commands-stack interval=900 N:$retcode" && exit 1
...
do
        STACK_STATUS="$(curl -X GET -g -k -q -s -L -o - --fail \
               ...
               --write-out "\n" \
               "$HEAT_ENDPOINT/$OS_PROJECT_ID/stacks/$stackname")" 
        STACK_STATUS="$(echo "$STACK_STATUS"|python3 -m json.tool|grep \"stack_status\"|awk -F: '{print $2}'|awk -F\" '{print $2}')"
...
[ $retcode -eq 0 ] && echo "PUTVAL $(hostname)/heat/commands-stack interval=900 N:$retcode" && echo "PUTVAL $(hostname)/heat/commands-stack_creation interval=900 N:$retcode" && exit 0
[ $retcode -ne 0 ] && echo "PUTVAL $(hostname)/heat/commands-stack interval=900 N:$retcode" && echo "PUTVAL $(hostname)/heat/commands-stack_creation interval=900 N:$retcode" && exit 1

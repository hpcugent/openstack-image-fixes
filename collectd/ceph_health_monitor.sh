...
TOKEN="$(curl -s -X POST "http://$CEPH_DASHBOARD_IP_PORT/api/auth" -H  "Accept: application/vnd.ceph.api.v1.0+json" -H  "Content-Type: application/json" \
	-d "{\"username\": \"$CEPH_DASHBOARD_USER\", \"password\": \"$CEPH_DASHBOARD_PASS\"}"|python3 -m json.tool|grep '"token":'|cut -d\" -f4)"
